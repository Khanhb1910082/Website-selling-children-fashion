...
if(isset($_POST['form_add_to_cart'])) {
	...
	// lấy hàng hiện tại của sản phẩm này
	$statement = $pdo->prepare("SELECT * FROM tbl_product WHERE p_id=?");
	$statement->execute(array($_REQUEST['id']));
	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
	foreach ($result as $row) {
	...
	if($_POST['p_qty'] > $current_p_qty):
		$temp_msg = 'Thông báo! Số lượng đặt hàng quá lớn!\nSố lượng sản phẩm hiện tại có '.$current_p_qty.'';
		?>
					...
					<div class="row">
						...
						<div class="col-md-7">
                            ...
                            <div class="p-price">
                                ...
                                <span>
                                    <?php if($p_old_price!=''): ?>
                                        <del><?php echo number_format($p_old_price,0,',','.'); ?>đ</del>
                                    <?php endif; ?> 
					...
					<div class="row">
						<div class="col-md-12">
							<ul class="nav nav-tabs" role="tablist">
                                ...
                                <li role="presentation"><a href="#condition" aria-controls="condition" role="tab" data-toggle="tab">Bảo quản</a></li>
                    ...
                    foreach ($result as $row) {
                        ...
                        <div class="item">
                            ...
                            <div class="text">
                                ...
                                <h4>
                                    <del>
                                        <?php echo number_format($row['p_old_price'],0,',','.'); ?>đ
                                    </del>
                                    <?php echo number_format($row['p_current_price'],0,',','.'); ?>đ
                                    <?php if($row['p_old_price'] != ''): ?>
                                    <?php endif; ?>
                                </h4>
                                
                                <p><a href="product.php?id=<?php echo $row['p_id']; ?>">Thêm vào giỏ</a></p>
                            </div>
                        </div>
                        <?php
                    }
