...
if(isset($_POST['form_about'])) {
    ...
    if(empty($_POST['about_content'])) {
        $valid = 0;
        $error_message .= 'Nội dung trang không được bỏ trống.<br>';
    }
    ...
    if($valid == 1) {
        ...
        if($path != '') {
            ...
            $statement = $pdo->prepare("UPDATE tbl_page SET about_title=?,about_content=?,about_banner=?,about_meta_title=?,about_meta_keyword=? WHERE id=1");
            $statement->execute(array($_POST['about_title'],$_POST['about_content'],$final_name,$_POST['about_meta_title'],$_POST['about_meta_keyword']));
        } else {
            $statement = $pdo->prepare("UPDATE tbl_page SET about_title=?,about_content=?,about_meta_title=?,about_meta_keyword=? WHERE id=1");
            $statement->execute(array($_POST['about_title'],$_POST['about_content'],$_POST['about_meta_title'],$_POST['about_meta_keyword']));
        }
...
if(isset($_POST['form_faq'])) {
    ...
    if($valid == 1) {
        ...
        if($path != '') {
            ...
            $statement = $pdo->prepare("UPDATE tbl_page SET faq_title=?,faq_banner=?,faq_meta_title=?,faq_meta_keyword=? WHERE id=1");
            $statement->execute(array($_POST['faq_title'],$final_name,$_POST['faq_meta_title'],$_POST['faq_meta_keyword']));
        } else {
            $statement = $pdo->prepare("UPDATE tbl_page SET faq_title=?,faq_meta_title=?,faq_meta_keyword=? WHERE id=1");
            $statement->execute(array($_POST['faq_title'],$_POST['faq_meta_title'],$_POST['faq_meta_keyword']));
        }

        $success_message = 'Trang FAQ đã được cập nhật thành công.';
...
if(isset($_POST['form_contact'])) {
    ...
    if($valid == 1) {
        if($path != '') {
            ...
            $statement = $pdo->prepare("UPDATE tbl_page SET contact_title=?,contact_banner=?,contact_meta_title=?,contact_meta_keyword=? WHERE id=1");
            $statement->execute(array($_POST['contact_title'],$final_name,$_POST['contact_meta_title'],$_POST['contact_meta_keyword']));
        } else {
            $statement = $pdo->prepare("UPDATE tbl_page SET contact_title=?,contact_meta_title=?,contact_meta_keyword=? WHERE id=1");
            $statement->execute(array($_POST['contact_title'],$_POST['contact_meta_title'],$_POST['contact_meta_keyword']));
        }

        $success_message = 'Trang liên hệ đã được cập nhật thành công.';  
    }
                        ...
                        <div class="tab-pane active" id="tab_1">
                            ...
                            <div class="box box-info">
                                <div class="box-body">
                                    ...
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Ảnh biểu ngữ</label>
                                    ...
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Từ khóa tìm kiếm </label>
                                        <div class="col-sm-8">
